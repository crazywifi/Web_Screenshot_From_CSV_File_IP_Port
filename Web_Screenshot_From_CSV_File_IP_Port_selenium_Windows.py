...
def take_screenshot(url, filename, retries=2):
    ...
    for attempt in range(retries):
        try:
            ...
            print(f"[WARNING] Timeout while accessing {url}. Retrying... ({attempt + 1}/{retries})")
        
        except WebDriverException as e:
            print(f"[ERROR] WebDriver error for {url}: {e}")
            return  # Exit if it's a severe error

    print(f"[ERROR] Failed to capture {url} after {retries} attempts.")
...
def main():
    ...
    try:
        ...
        driver.quit()
        print("[OK] Finished capturing screenshots.")
